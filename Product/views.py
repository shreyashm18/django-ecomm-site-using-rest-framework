...
@login_required(login_url = 'user_login')
def detail_prod(request,prod_id):
    print(type(prod_id))
    prod_id=int(prod_id)
    try:
    ...
    return render(request, 'detail_prod.html', {"product":product,})
...
def add_category(request):
    ...
    if request.method=='POST':
        ...
        return render(request, 'category_add.html', {'upload_form':category,"title":'Add New Category to your shopping site',})
...
def all_carts(request):
    if request.method == "GET":
        form = SelectCustomer()
        return render(request, 'select_cust.html',{'form':form})
    elif request.method == "POST":
        form = SelectCustomer(request.POST)
